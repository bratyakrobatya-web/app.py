...
<p><span class="step-number">1</span> <strong>Обычный режим</strong></p>

- Загрузите файл Excel с колонкой "Город"
- Система автоматически сопоставит города со справочником HH.ru
- Отредактируйте города с совпадением <90%
- Скачайте результат одним файлом

<p><span class="step-number">2</span> <strong>Режим с колонкой "Вакансия"</strong></p>

- Загрузите файл с колонками "Город" и "Вакансия"
- Данные автоматически разделятся по вакансиям
- **Выберите режим работы:**
  - ⚡ **Единая сверка гео** - распознайте города один раз, изменения применятся ко всем вакансиям (рекомендуется при ≥3 вакансиях)
  - 📋 **По вакансиям** - редактируйте каждую вакансию отдельно
- Скачайте архив со всеми файлами или отдельные файлы

<p><span class="step-number">3</span> <strong>Режим с вкладками</strong></p>

- Загрузите Excel с несколькими вкладками (или с названиями "вакансия...")
- Каждая вкладка обрабатывается отдельно
- **Выберите режим работы:**
  - ⚡ **Единая сверка гео** - все вкладки используют одни изменения
  - 📋 **По вкладкам** - редактируйте каждую вкладку отдельно
- Идеально для работы с несколькими наборами данных

**Порядок работы:**
1. Загрузите файл → 2. Нажмите "Начать сопоставление" → 3. Выберите режим (если есть вакансии/вкладки) → 4. Отредактируйте города <90% → 5. Скачайте результат
        """,
...
with st.expander("Яндекс.Еда", expanded=False):
    ...
    **ВАЖНО!** Скрипт нужно запускать в Google Colab.
    ...
    colab_url_yaeda = "https://colab.research.google.com/github/bratyakrobatya-web/app.py/blob/main/yaeda_colab.ipynb"
    st.markdown(f"""
    <a href="{colab_url_yaeda}" target="_blank">
    ...
    1. **Запустите первую ячейку** (установка библиотек) и дождитесь завершения
    2. **Запустите вторую ячейку** (основной код)
    3. Загрузите файлы когда появится запрос
