...
with st.sidebar:
    ...
    <a href="#сверки-с-клиентами" class="nav-link">Сверки с клиентами</a>
    """, unsafe_allow_html=True)
...
# Сверка Я.Еда
with st.expander("🍔 Сверка Я.Еда", expanded=False):
    st.markdown("""
    ### 📋 Инструкция

    Ожидаются файлы с названием:
    - **"ООО Хэдхантер Биллинг....."** - отчет биллинг
    - **"Отчет-по-откликам-по-проектам-работодателя-"** - отчет внутренний hh
    - **"Leads_"** - лиды из ЛК Я.Еды
    """)

    if st.button("🚀 Перейти к скрипту", type="primary", use_container_width=True, key="yandex_eda_script"):
        st.write("Перенаправление на скрипт...")
        st.markdown("""
        <meta http-equiv="refresh" content="0; url=https://colab.research.google.com/drive/1nETa93X1nDdyQ4b5AL7W6AsqPdXnCxQj?usp=sharing">
        <script>window.open('https://colab.research.google.com/drive/1nETa93X1nDdyQ4b5AL7W6AsqPdXnCxQj?usp=sharing', '_blank');</script>
        """, unsafe_allow_html=True)
